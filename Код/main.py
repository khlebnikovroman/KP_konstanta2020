...
# Подключение файла с интерфейсом
...
class MainWindow(TemplateBaseClass):
    ...
    def __init__(self):
        ...
        self.setWindowTitle('Расчет константы скорости химической рреакции 2A= 2B + C')
        ...
        self.ui.tableWidget_2.setHorizontalHeaderLabels(["t, с", "Ca расч., моль/л", "Cb, моль/л", "Cc, моль/л"])
...
Для начала работы введите данные об исходных веществах: начальныее концетрации Cb и Cc, заполните таблицу с измененим концетрации Ca в течении времени.
После ввода начальных данных нажмите кнопку "Провести расчёты" и вы получите результаты.
Автор программы: Хлебников Роман Александрович
Группа: 494
        ...
        try:
            ...
            for i in range(spinboxvalue):
                ...
                if i == 0:
                    ...
                    cb = cb + self.k * (ca ** self.n) * t
                    cc = cc + self.k * (ca ** self.n) * t * 2
            ...
            # отрисовка линий
            self.ui.plot.plot(series1[0], series1[1], clear=True, pen='#000DFF', symbolPen='#000DFF', name='Ca эксп')
            self.ui.plot.plot(series2[0], series2[1], pen=(0,0,0,0), symbolBrush="r", name='Ca расч.')
            self.ui.plot.plot(series3[0], series3[1], pen='#A600A0', symbolPen='#A600A0', name='Cb')
            self.ui.plot.plot(series4[0], series4[1], pen='#A60600', symbolPen='#A60600', name='Cс эксп')
            # вывод сообщения
            msg = QMessageBox()
            msg.setIcon(QMessageBox.Information)
